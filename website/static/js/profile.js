...
const $profileButtons = document.getElementsByName('prof');

let ansNumOfMembers = -1;

console.log($profileButtons[0].style.backgroundColor);
let Answer;
// クイズの問題文、選択肢を定義
const createAnswers = () => {
     questions = [];
     // buttonによってquestionに何を採用するか決める。
     let profloopIndex = 0;
     while(profloopIndex < 4){
          if($profileButtons[profloopIndex].style.backgroundColor === 'silver'){
               questions.push($profileButtons[profloopIndex].textContent);
          }
          profloopIndex++;
     }
...
let questionIndex = 0;
while (questionIndex < 4) {
     $profileButtons[questionIndex].addEventListener('click', (e) => {
          changeQuestions(e);
     });
     questionIndex++;
};
